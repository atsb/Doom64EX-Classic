...
void AM_Ticker(void) {
    ...
    if(followplayer) {
        if(am_flags & AF_PANMODE) {
            float panscalex = v_msensitivityx.value / (1500.0f / scale);
            float panscaley = v_msensitivityy.value / (1500.0f / scale);

            automappanx += (fixed_t)((I_MouseAccel(mpanx) * panscalex) * (FRACUNIT / 128.0f));
            automappany += (fixed_t)((I_MouseAccel(mpany) * panscaley) * (FRACUNIT / 128.0f));
            mpanx = mpany = 0;
        }
        else {
            automapx = plr->mo->x;
